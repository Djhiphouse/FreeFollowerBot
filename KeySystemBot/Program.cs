	...
	internal class Program
	{
		...
		private async Task<Task> CommandHandler(SocketMessage message)
		{
			try
			{
				...
				if (!message.Channel.Name.StartsWith("server"))
					return Task.CompletedTask;

				if (message.Content.StartsWith("!addperms") && message.Author.Id == 1067545233811849227 || UserPerms.Contains(message.Author.Id))
				{
